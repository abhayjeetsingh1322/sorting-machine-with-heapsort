...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void exchangeEntries(T[] array, int i, int j) {
        ...
        //Checking if the entries are not equal
        if (i != j) {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        //Compare left and right, to avoid comparing nulls we add if less than last
        //for full tree structure
        if (rightChild <= last) {

            int compareInt = order.compare(array[leftChild], array[rightChild]);

            //If left child is smaller
            if (compareInt < 0) {
                exchangeEntries(array, leftChild, top);
                siftDown(array, leftChild, last, order);

                //Meaning the right child is smaller
                //CORRECTION: ADDED ELSE IF FOR OTHER CASES
            } else if (compareInt > 0) {
                exchangeEntries(array, rightChild, top);
                siftDown(array, rightChild, last, order);
            }

            //Else if for an incomplete tree
        } else if (leftChild <= last) {

            //Compare root and the left child (only child)
            int compareInt = order.compare(array[top], array[leftChild]);

            //If root is bigger than swapping
            if (compareInt > 0) {
                exchangeEntries(array, leftChild, top);
                siftDown(array, leftChild, last, order);
            }
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        //Recursively heapify subtrees (1)
        //Heapify subtrees for a complete tree
        if (rightChild <= array.length - 1) {
            heapify(array, leftChild, order);
            heapify(array, rightChild, order);

            //Heapify subtrees for incomplete tree
        } else if (leftChild <= array.length - 1) {
            heapify(array, leftChild, order);
        }
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        //Creating new array of generic types
        T[] heap = (T[]) (new Object[q.length()]);

        //Getting length of the queue of entries
        int length = q.length();

        //Entering for loop that puts entries of queue in array
    ...
    private void createNewRep(Comparator<T> order) {
        ...
        //Making every field in its default state
    ...
    public final void changeToExtractionMode() {
        ...
        //Building the heap from the current entries in the queue
        this.heap = buildHeap(this.entries, this.machineOrder);

        //Changing heap size to match the length of array
        this.heapSize = this.heap.length;

        //Finally changing insertion mode
    ...
    public final T removeFirst() {
        ...
        //Getting the first element of the heap
        T firstElement = this.heap[0];

        //Exchanging the position of last entry and the first entry
        exchangeEntries(this.heap, 0, this.heapSize - 1);

        //Decreasing size of heap (also it "removes" the last entry,
    ...
    public final int size() {
        ...
        //Declaring variable to hold size
        int size = 0;

        //If the sorting machine is in insertion mode, then making size
        //the size of the queue of entries
        if (this.insertionMode) {
            size = this.entries.length();

            //Else means the sorting machine isn't in insertion mode, then
            //making size equal the size of the array
        } else {
            size = this.heapSize;
        }
